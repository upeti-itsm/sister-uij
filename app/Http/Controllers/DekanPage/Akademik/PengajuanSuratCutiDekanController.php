...
class PengajuanSuratCutiDekanController extends Controller
{
    public function index()
    {
        $menu = 'Persetujuan Surat Cuti';
        return view('dekan_page.akademik.pengajuan_surat.pengajuan_surat_cuti', compact('menu'));
    }
    ...
    public function json(Request $request)
    {
        try {
            ...
            $result = PengajuanSurat::get_list_dekan($idPersonal, $status, $search, $start, $length);
    ...
    public function detail(Request $request)
    {
        try {
            ...
            $idPersonal = $user->id_personal ?? null;
            if (!$idPersonal) {
                return response()->json(['status' => '0', 'keterangan' => 'ID personal dekan tidak ditemukan'], 422);
            }

            $detail = PengajuanSurat::get_detail_with_access($idPengajuan, $idPersonal);
    ...
    public function approve(Request $request)
    {
        try {
            ...
            if (!$detail || (string) ($detail->status_pengajuan ?? '') !== '2') {
                return response()->json(['status' => '0', 'keterangan' => 'Pengajuan tidak dapat disetujui, status tidak valid atau Anda tidak memiliki akses'], 422);
            }

            $result = PengajuanSurat::set_status($idPengajuan, '4', $idPersonal, $request->catatan ?? null);
    ...
    public function reject(Request $request)
    {
        try {
            ...
            $detail = PengajuanSurat::get_detail_with_access($idPengajuan, $idPersonal);
            if (!$detail || (string) ($detail->status_pengajuan ?? '') !== '2') {
                return response()->json(['status' => '0', 'keterangan' => 'Pengajuan tidak dapat ditolak, status tidak valid atau Anda tidak memiliki akses'], 422);
            }

            $result = PengajuanSurat::set_status($idPengajuan, '5', $idPersonal, $catatan);
    ...
    private function withStatusLabel($rows)
    {
        return collect($rows)
            ->map(function ($row) {
                $row->keterangan_status = PengajuanSurat::status_label($row->status ?? null);
                return $row;
            })
            ->values();
    }
