...
class PengajuanSuratAktifController extends Controller
{
    public function index()
    {
        $menu = 'Pengajuan Surat Aktif';
        ...
        return view('mahasiswa_page.akademik.pengajuan_surat.pengajuan_surat_aktif', compact('menu', 'user', 'semesterAktif'));
    }
    ...
    public function json_pengajuan_surat(Request $request)
    {
        try {
            ...
            $result = PengajuanSurat::get_list_aktif_mahasiswa($nim, $status, $jenis, $search, $start, $length);
    ...
    public function store_pengajuan_surat(Request $request)
    {
        try {
            ...
            $result = PengajuanSurat::insup_pengajuan_aktif(
                $nim,
                $request->keperluan,
                $tahunAkademik
            );
    ...
    public function delete_pengajuan_surat(Request $request)
{
    ...
    public function download_surat($id_pengajuan)
    {
        try {
            ...
            $detail = PengajuanSurat::get_detail_aktif($id_pengajuan);
            if (!$detail) {
                return response()->json(['status' => '0', 'keterangan' => 'Pengajuan tidak ditemukan'], 404);
            }
            ...
            $mahasiswa = (object) [
                ...
                'alamat'         => $detail->alamat         ?? $detail->alamat_lengkap ?? '-',
                'no_hp'          => $detail->no_hp          ?? '-',
                'tahun_akademik' => $detail->tahun_akademik ?? '-',
                'semester_aktif' => $detail->semester_aktif ?? '-',
                'keperluan'      => $detail->keperluan      ?? '-',
            ];

            $tanggalCetak = Carbon::now()->format('d F Y');

            $pdf = Facade::loadView('mahasiswa_page.akademik.pengajuan_surat.pdf_aktif', [
                'pengajuan'          => $detail,
                'mahasiswa'          => $mahasiswa,
                'logo'               => $logo,
            ...
            $fileName = 'surat-aktif-akademik-' . $detail->nim . '.pdf';
