        ...
        @for($i=0; $i < sizeof($pendaftar); $i+=3)
            <tr>
                <td class="center" style="width: 40%; vertical-align: top; text-align: center">
                    <img
                        src="data:image/png;base64,{{\Milon\Barcode\Facades\DNS1DFacade::getBarcodePNG($pendaftar[$i]->code_barcode, 'C128')}}"
                        alt="barcode" style="height: 30px; width: 100%; margin: 1px"/>
                    <h5 style="margin: 1px">{{$pendaftar[$i]->code_barcode}}</h5>
                    <h4 style="margin-top: 15px; margin-bottom: 1px">{{$pendaftar[$i]->nama_mahasiswa}}</h4>
                    <h1 style="margin: 1px">{{$pendaftar[$i]->nomor_kursi}}</h1>
                </td>
                <td class="center" style="width: 40%; vertical-align: top; text-align: center">
                    @if(isset($pendaftar[$i+1]))
                        ...
                        <h4 style="margin-top: 15px; margin-bottom: 1px">{{$pendaftar[$i+1]->nama_mahasiswa}}</h4>
                        <h1 style="margin: 1px">{{$pendaftar[$i+1]->nomor_kursi}}</h1>
                    @endif
                </td>
                <td class="center" style="width: 40%; vertical-align: top; text-align: center">
                    @if(isset($pendaftar[$i+2]))
                        ...
                        <h4 style="margin-top: 15px; margin-bottom: 1px">{{$pendaftar[$i+2]->nama_mahasiswa}}</h4>
                        <h1 style="margin: 1px">{{$pendaftar[$i+2]->nomor_kursi}}</h1>
                    @endif
                </td>
            </tr>
        @endfor
