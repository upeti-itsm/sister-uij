    ...
    <div class="col-sm-8 header-title p-0">
        <div class="media">
            ...
            <div class="media-body">
                ...
                <small>Halaman ini digunakan untuk minitoring data mahasiswa pada database siakad</small>
            </div>
        </div>
    </div>
@endsection
                                        ...
                                        <div class="col-md-5">
                                            <select class="select2 form-control" id="angkatan">
                                                @foreach($angkatan AS $item)
                                                    <option value="{{trim($item->thnakademikmasuk)}}">{{trim($item->thnakademikmasuk)}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            <select class="select2 form-control" id="status">
                                                @foreach($status_aktif AS $item)
                                                    <option value="{{$item->kdstatusakademik}}">{{trim($item->statusakademik)}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3">
...
@push('scripts')
    ...
    <script src="{{asset('adminpage/own-js/super_admin/siakad/mahasiswa.js')}}"></script>
@endpush
