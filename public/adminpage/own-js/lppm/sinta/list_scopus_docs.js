jQuery.list_doc_author = {
    ...
    setEvents: function () {
        ...
        self.data.table = $("#table").DataTable({
            serverSide: true,
            ajax: {
                url: '/lppm/sinta/list-scopus-docs-json',
                type: 'post',
                data: function (data) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        return "<b>" + data.creator + "</b><br/>" +
                            "<small>Publisher: " + data.publication_name + "</small><hr/>" +
                            "<span class='badge badge-danger-soft mr-2'>Q-" + data.quartile + "<br/><small>Accreditation</small></span>" +
                            "<span class='badge badge-success-soft mr-2'>" + data.cover_display_date + "<br/><small>Publish Date</small></span>";
                    }
                },
                {
                    data: null,
                ...
                },
                {
                    ...
                    width: "10%",
                    render: function (data) {
                        return "<button class='btn btn-sm btn-success-soft'>" + data.citedby_count + "</button>";
                    }
                }
            ],
            paging: true,
                ...
                buttons: {
                    confirm: {
                        ...
                        action: function () {
                            ...
                            $("#row-list-data").hide();
                            $.ajax({
                                url: '/lppm/sinta/get-scopus-docs-json',
                                method: 'post',
                                data: {
    ...
    next_data: function (data, id_sinta, index = 0, progres = 0, failed = 0, inserted = 0) {
        ...
        var n = data.length;
        $.ajax({
            url: '/lppm/sinta/store-scopus-docs-json',
            method: 'post',
            data: {
                'id_sinta': id_sinta,
                'doc_id_sinta': data[index].id,
                'quartile': data[index].quartile,
                'title': data[index].title,
                'publication_name': data[index].publication_name,
                'creator': data[index].creator,
                'page': data[index].page,
                'issn': data[index].issn,
                'volume': data[index].volume,
                'cover_date': data[index].cover_date,
                'cover_display_date': data[index].cover_display_date,
                'doi': data[index].doi,
                'citedby_count': data[index].citedby_count,
                'aggregation_type': data[index].aggregation_type,
                'url': data[index].url,
            },
            success: function (result) {
                if (result.status == 1) {
