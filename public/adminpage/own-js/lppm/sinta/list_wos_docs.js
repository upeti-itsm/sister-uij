jQuery.list_doc_author = {
    ...
    setEvents: function () {
        ...
        self.data.table = $("#table").DataTable({
            serverSide: true,
            ajax: {
                url: '/lppm/sinta/list-wos-docs-json',
                type: 'post',
                data: function (data) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        return "<small style='font-weight: bold'>" + data.journal_name + "</small><br/>" +
                            "<small>" + data.authors + "</small><hr/>" +
                            "<span class='badge badge-danger-soft mr-2'>" + data.issn + "<br/><small>ISSN</small></span>" +
                ...
                buttons: {
                    confirm: {
                        ...
                        action: function () {
                            ...
                            $("#row-list-data").hide();
                            $.ajax({
                                url: '/lppm/sinta/get-wos-docs-json',
                                method: 'post',
                                data: {
    ...
    next_data: function (data, id_sinta, index = 0, progres = 0, failed = 0, inserted = 0) {
        ...
        var n = data.length;
        $.ajax({
            url: '/lppm/sinta/store-wos-docs-json',
            method: 'post',
            data: {
                'id_sinta': id_sinta,
                ...
                'first_author': data[index].first_author,
                'last_author': data[index].last_author,
                'authors': data[index].authors,
                'publish_date': data[index].publish_date,
                'publish_type': data[index].publish_type,
                'publish_year': data[index].publish_year,
                'journal_name': data[index].journal_name,
                'abstract': data[index].abstract,
                'citation': data[index].citation,
                'page_begin': data[index].page_begin,
                'page_end': data[index].page_end,
                'issn': data[index].issn,
                'eissn': data[index].eissn,
                'url': data[index].url,
            },
            success: function (result) {
                if (result.status == 1) {
