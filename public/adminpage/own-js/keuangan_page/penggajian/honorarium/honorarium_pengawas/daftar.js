    ...
    setEvents: function () {
        ...
        $("#btn_buat_honor").click(function () {
            ...
            $.confirm({
                ...
                title: '',
                columnClass: 'medium',
                content: 'Apakah anda yakin akan membuat Honorarium Pengawas periode <b>' + self.data.bulan[dateObj.getMonth()] + '</b> tahun <b>' + dateObj.getFullYear() + '</b> ?',
                buttons: {
                    export: {
                        text: 'Yakin',
                        btnClass: 'btn-blue',
                        action: function () {
                            $.ajax({
                                url: '/keu/penggajian/honorarium/honorarium-pengawas/set-honor',
                                method: 'post',
                                data: {},
                                beforeSend: function () {
        ...
        self.data.table = $("#table").DataTable({
            serverSide: true,
            ajax: {
                url: '/keu/penggajian/honorarium/honorarium-pengawas/json',
                type: 'post',
                data: function (data) {
                ...
                },
                {
                    ...
                    render: function (data) {
                        if (data.is_repair === false)
                            return "<a href='/keu/penggajian/honorarium/honorarium-pengawas/detail/" + data.id_rekapitulasi_gaji_bulanan_pegawai + "' title='Lihat Detail Gaji' class='btn btn-sm btn-primary btn-edit mr-2' data-id='" + data.id_rekapitulasi_gaji_bulanan_pegawai + "'><i class='fas fa-search'></i></a>" +
                                "<a href='/keu/penggajian/honorarium/honorarium-pengawas/slip-gaji/" + data.id_rekapitulasi_gaji_bulanan_pegawai + "' title='Print Slip Gaji' class='btn btn-sm btn-info btn-print mr-2' data-id='" + data.id_rekapitulasi_gaji_bulanan_pegawai + "'><i class='fas fa-print'></i></a>";
                        else
                            return "<a href='/keu/penggajian/honorarium/honorarium-pengawas/detail/" + data.id_rekapitulasi_gaji_bulanan_pegawai + "' title='Lihat Detail Gaji' class='btn btn-sm btn-danger btn-edit mr-2' data-id='" + data.id_rekapitulasi_gaji_bulanan_pegawai + "'><i class='fas fa-exclamation-triangle mr-2'></i>Butuh Perbaikan</a>"
                    }
                },
                {
                    data: 'nama',
