jQuery.insentif_lainnya = {
    ...
    setEvents: function () {
        ...
        $("#btn_upload_insentif").click(function () {
            $("#modal-upload-insentif-lainnya").modal('show');
        });

        self.data.table = $("#table").DataTable({
            serverSide: true,
            ajax: {
                url: '/keu/penggajian/pengaturan-gaji/insentif-lainnya/json',
                type: 'post',
                data: function (data) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        return "<p><b>" + data.informasi_insentif_lainnya + "</b>" +
                            "</p>";
                    }
                },
                {
                ...
                },
                {
                    ...
                    width: "30%",
                    render: function (data) {
                        return "<a class='btn btn-primary btn-sm mr-2' title='Detail Insentif' href='/keu/penggajian/pengaturan-gaji/insentif-lainnya/detail/" + data.periode_rekap + "/" + data.tahun + "'><i class='fas fa-eye mr-2'></i>Detail</a>" +
                            "<a target='_blank' class='btn btn-danger btn-sm' title='Export Peserta' href='/keu/penggajian/pengaturan-gaji/insentif-lainnya/export/pdf/" + data.periode_rekap + "/" + data.tahun + "'><i class='fas fa-file-pdf mr-2'></i>Download</a>";
                    }
                }
            ],
            paging: true,
        ...
        $("#btn-submit-file").click(function () {
            if ($("#add-bulan").val() && $("#add-tahun").val() && $("#customFile").val()) {
                $.confirm({
                    title: 'Konfirmasi !',
                    type: 'orange',
                    content: 'Apakah anda yakin insentif yang anda upload sudah benar ? <b style="color: red">Akan Berpengaruh pada Gaji Pegawai</b>',
                    buttons: {
                        confirm: {
                            text: 'Yakin',
                            btnClass: 'btn-green',
                            ...
                            action: function () {
                                ...
                                $("#modal-upload-potongan-lainnya").modal('hide');
                                ...
                                $.ajax({
                                    url: '/keu/penggajian/pengaturan-gaji/insentif-lainnya/upload',
                                    type: 'POST',
                                    data: formData,
    ...
    next_data: function (data, periode, tahun, index = 1, progres = 0, failed = 0, inserted = 0) {
        var self = this;
        var n = data.length;
        $.ajax({
            url: '/keu/penggajian/pengaturan-gaji/insentif-lainnya/insert',
            method: 'post',
            data: {
                'id_karyawan': data[index][0],
                'insentif': data[index][2],
                'periode': periode,
                'tahun': tahun,
                'keterangan': data[index][3]
            },
            ...
            error: function () {
                self.data.log_table.row.add([
                    (index + 1),
                    data[index][1] + " (" + data[index][2] + ")",
                    "<i class='fas fa-times-circle text-danger p-1'></i> Data Kosong",
                    data[index][1],
                    data[index][1],
                    "gagal"
                ]).draw();
