jQuery.potongan_qurban = {
    data: {
        table: $("#table"),
        tgl_mulai_qurban: $("#tgl_mulai_qurban"),
        tgl_selesai_qurban: $("#tgl_selesai_qurban"),
    ...
    setEvents: function () {
        ...
        // Option Data
        $(".select2").select2();
        self.data.tgl_mulai_qurban = $("#tgl_mulai_qurban").datepicker({
            language: 'id',
            format: 'dd MM yyyy',
            autoclose: true,
            ...
            self.data.tgl_selesai_qurban.datepicker('setDate', moment(e.date).format('D/M/YYYY'));
            self.data.tgl_selesai_qurban.datepicker('setStartDate', moment(e.date).format('D/M/YYYY'));
        });

        self.data.tgl_selesai_qurban = $("#tgl_selesai_qurban").datepicker({
            language: 'id',
            format: 'dd MM yyyy',
            autoclose: true,
            startDate: moment().format('D/M/YYYY'),
        ...
        $("#btn_upload_potongan").click(function () {
            $("#modal-upload-potongan-qurban").modal('show');
        });

        self.data.table = $("#table").DataTable({
            serverSide: true,
            ajax: {
                url: '/keu/penggajian/pengaturan-gaji/potongan-qurban/json',
                type: 'post',
                data: function (data) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        return "<p><b>" + data.informasi_potongan + "</b>" +
                            "</p>";
                    }
                },
                {
                ...
                },
                {
                    ...
                    width: "30%",
                    render: function (data) {
                        return "<a class='btn btn-primary btn-sm mr-2' href='/keu/penggajian/pengaturan-gaji/potongan-qurban/detail/" + data.periode_rekap + "/" + data.tahun + "' title='Detail Potongan'><i class='fas fa-eye mr-2'></i>Detail</a>" +
                            "<a target='_blank' class='btn btn-danger btn-sm' href='/keu/penggajian/pengaturan-gaji/potongan-qurban/export/pdf/" + data.periode_rekap + "/" + data.tahun + "' title='Export Peserta'><i class='fas fa-file-pdf mr-2'></i>Download</a>";
                    }
                }
            ],
            paging: true,
        ...
        $("#btn-submit-file").click(function () {
            if ($("#tgl_mulai_qurban").val() && $("#tgl_selesai_qurban").val() && $("#customFile").val()) {
                $.confirm({
                    title: 'Konfirmasi !',
                    type: 'orange',
                    ...
                    buttons: {
                        confirm: {
                            ...
                            action: function () {
                                ...
                                $("#modal-upload-potongan-qurban").modal('hide');
                                ...
                                $.ajax({
                                    url: '/keu/penggajian/pengaturan-gaji/potongan-qurban/upload',
                                    type: 'POST',
                                    data: formData,
                                    processData: false,  // tell jQuery not to process the data
                                    contentType: false,  // tell jQuery not to set contentType
                                    success: function (data) {
                                        self.next_data(data, moment(self.data.tgl_mulai_qurban.datepicker('getDate')).format('YYYY-MM-DD'), moment(self.data.tgl_selesai_qurban.datepicker('getDate')).format('YYYY-MM-DD'));
                                    }
                                });
                            }
                        },
    ...
    next_data: function (data, awal, akhir, index = 0, progres = 0, failed = 0, inserted = 0) {
        ...
        var n = data.length;
        $.ajax({
            url: '/keu/penggajian/pengaturan-gaji/potongan-qurban/insert',
            method: 'post',
            data: {
