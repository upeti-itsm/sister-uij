jQuery.pengajuanSuratDekan = {
    ...
    initTable: function () {
        ...
        self.data.table = $('#table-pengajuan-surat-dekan').DataTable({
            ...
            searching: false,
            ajax: {
                url: '/dekan/pengajuan-surat-cuti/json',
                type: 'POST',
                beforeSend: function () {
            ...
            columns: [
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function (data) {
                        var id = data.id_riwayat_pengajuan_cuti_mahasiswa;
    ...
    bindEvents: function () {
        ...
        $('#table-pengajuan-surat-dekan').on('click', '.btn-detail-dekan', function () {
            ...
            $.ajax({
                url: '/dekan/pengajuan-surat-cuti/detail',
                method: 'POST',
                data: {
                ...
                success: function (response) {
                    if (response.status === '1') {
                        ...
                        // DEBUG: uncomment baris berikut jika nama dosen PA masih '-'
                        // untuk melihat field apa saja yang dikembalikan server
                        // console.log('[DEBUG] response.data keys:', Object.keys(d));
                        // console.log('[DEBUG] nama_dosen_pa:', d.nama_dosen_pa);
                        ...
                        $('#detail-dekan-tahun-akademik').text(d.tahun_akademik || '-');
                        $('#detail-dekan-tgl').text(d.tgl_created || '-');
                        $('#detail-dekan-dpa').text(d.nama_dpa || '-');
        ...
        $('#btn-approve-dekan').on('click', function () {
            $.confirm({
                ...
                content: 'Setujui pengajuan surat ini?',
                type: 'green',
                buttons: {
                    cancel: { text: 'Batal' },
                    confirm: {
                        ...
                        action: function () {
                            $.ajax({
                                url: '/dekan/pengajuan-surat-cuti/approve',
                                method: 'POST',
                                data: {
                                    _token: $('meta[name="csrf-token"]').attr('content'),
                                    id_pengajuan: currentId
        ...
        $('#btn-reject-dekan').on('click', function () {
            $.confirm({
                ...
                content: 'Apakah Anda yakin ingin menolak pengajuan surat ini?',
                type: 'red',
                buttons: {
                    cancel: { text: 'Batal' },
                    confirm: {
                        ...
                        action: function () {
                            $.ajax({
                                url: '/dekan/pengajuan-surat-cuti/reject',
                                method: 'POST',
                                data: {
                                    _token: $('meta[name="csrf-token"]').attr('content'),
                                    id_pengajuan: currentId,
...
$(document).ready(function () {
    if ($('#table-pengajuan-surat-dekan').length) {
        jQuery.pengajuanSuratDekan.init();
    }
});
