jQuery.hasil_kuesioner = {
    ...
    setEvents: function () {
        ...
        $("#id_jenis").change(function () {
            $.ajax({
                url: "/bpm/kuesioner/kuesioner-kepuasan-mahasiswa/get-unsur/" + $(this).val(),
                method: 'get',
                success: function (response) {
                    $("#id_unsur").html("");
                    var html = "";
                    $.each(response, function (index, item) {
                        html = html + "<option value='" + item.id_unsur + "'>" + item.unsur_penilaian + "</option>"
                    });
                    $("#id_unsur").append(html);
                },
                beforeSend: function () {
                    $("#id_unsur").hide();
                    $("#loading-unsur").show();
        ...
        $("#btn-kelola-kuesioner").click(function () {
            location.href = "/bpm/kuesioner/kuesioner-kepuasan-mahasiswa/export/" + $("#id_jenis").val() + '/' + $("#id_semester").val()
        });
    ...
    reload_unsur: function () {
        ...
        $("#sub-unsur-area").html("");
        $.ajax({
            url: "/bpm/kuesioner/kuesioner-kepuasan-mahasiswa/json",
            method: 'post',
            data: {
            ...
            success: function (response) {
                $.each(response, function (index, val) {
                    self.add_sub_unsur(val.id_sub_unsur_kuesioner_kepuasan_mahasiswa, index + 1, val.pernyataan);
                    var data = [
                        {
                    ...
                    self.new_chart(val.id_sub_unsur_kuesioner_kepuasan_mahasiswa, data);
                });
            },
            complete: function () {
                $("#loading").hide();
